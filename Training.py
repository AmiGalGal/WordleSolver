#Disclaimer: using RL is not the recommended way to achieve this, this is for edu purposes, the recommended is probably minimax or some info-theoretic search
#But I don't mess with information theory that much.
#it does get 100% but it still overkill for this
        ...
        for i, letter in enumerate(guess):
            ...
            if feedback[i] == 2: #keeps the 2s as 2s
                ...
                for pos in range(self.word_length):
                    if self.obs_matrix[idx, pos] != 2:
                        self.obs_matrix[idx, pos] = 1

    def _compute_reward(self, feedback):
        if all(f == 2 for f in feedback):
            return 10 #if the guess was correct return this reward
        reward = -1 #for every guess small penalty
        reward += sum([0.5 if f > 0 else 0 for f in feedback]) #partial credit for green or yellow letters
        return reward
...
solved_count = 0 #instead of using abs score to beat will be stupid since some words are impossible to achieve high scores (needs 5 anagrams of the word)
...
for episode in range(1, number_episodes + 1):
  ...
  print('\repisode {}\taverage score: {:.2f}\tsolved rate: {:.2f}%'.format(
      episode, np.mean(scores_on_100_episodes), (solved_count / episode) * 100
  ), end="")
  if episode % 100 == 0:
      print('\repisode {}\taverage score: {:.2f}\tsolved rate: {:.2f}%'.format(
          episode, np.mean(scores_on_100_episodes), (solved_count / episode) * 100
  ...
  if episode >= 100:
      ...
      if solved_rate >= 0.95:  # stop if agent guesses correctly 90% of the time
          print('\nenvironment solved in {:d} episodes!\tsolved Rate: {:.2f}%'.format(
              episode, solved_rate * 100
          ))
