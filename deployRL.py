...
state_size = 26 * 5
number_actions = N  # size of your WordBank
...
print("Wordle AI ready! Enter feedback after each guess:")
print("2 = correct, 1 = present, 0 = absent")
print("Example: '20110' means first letter correct, second absent, third correct, fourth correct, fifth absent")

guess_count = 0
max_guesses = 6
done = False
env = WordleEnv(WordBank)
state = env.reset()
state_flat = state.flatten()

while not done and guess_count < max_guesses:
    # Agent selects the best action (no epsilon for testing)
    action = agent.act(state_flat, epsilon=0.0)
    guess = WordBank[action]
    print(f"\nAgent guesses ({guess_count+1}/{max_guesses}): {guess}")

    # Ask user for feedback
    while True:
        feedback_str = input("Enter feedback (5 digits 0/1/2): ").strip()
        if len(feedback_str) == 5 and all(c in '012' for c in feedback_str):
            break
        print("Invalid input! Must be 5 digits using only 0,1,2.")

    # Convert feedback to list of integers
    feedback = [int(c) for c in feedback_str]

    # Update environment manually
    env._update_obs_matrix(guess, feedback)
    state_flat = env.obs_matrix.flatten()

    # Update counters
    guess_count += 1
    done = all(f == 2 for f in feedback)

if done:
    print(f"\nWord guessed correctly in {guess_count} guesses!")
else:
    print(f"\nMax guesses reached ({max_guesses}). Word not solved.")
